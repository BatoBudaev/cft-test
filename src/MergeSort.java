...
public class MergeSort {
    public static void doIntMergeSort(ArgumentsData data) throws IOException {
        ...
        for(int i = 0; i < filesCount; ++i) {
            readers[i] = new Reader((String)data.getInputNamesArray().get(i));
        }
        ...
        while(hasLines) {
            ...
            for(int i = 0; i < filesCount; ++i) {
                if (readers[i].checkNextLine() && readers[i].isReading()) {
                    ...
                    } else if (data.getSortMode().equals("-d") && number < readers[i].getNumberValue()) {
                        number = readers[i].getNumberValue();
                        index = i;
                    }
        ...
        while(hasLines) {
            ...
            for(int i = 0; i < filesCount; ++i) {
                if (readers[i].checkNextLine() && readers[i].isReading()) {
                    ...
                    } else if (data.getSortMode().equals("-d") && stringValue.compareTo(readers[i].getStringValue()) < 0) {
                        stringValue = readers[i].getStringValue();
                        index = i;
                    }
